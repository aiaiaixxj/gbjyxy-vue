  ...
  <div id="app">
    ...
    <div class="login-container">
      ...
      <div class="fixed-bar">
        ...
        <p>
          Powered by JeeCms Copyright © 2007-2017 www.jeecms.com, All Rights
          Reserved
        </p>
      </div>
    </div>
...
<script>
import $ from "jquery";
import CanvasAnimate from "@/components/plugs/canvas";
export default {
  name: "login",
  data() {
    return {
  ...
  methods: {
    ...
    requireUserName() {
      if (this.params.userName == "") {
        $("#userName").addClass("error").text("请填写用户名");
      } else {
    ...
    requirePwd() {
      if (this.params.password == "") {
        $("#pwd").addClass("error").text("请输入密码");
      } else {
        $("#pwd").addClass("error").text(" ");
      }
    },
    login() {
      this.loading = true;
